...
import { test as withCssCoverage } from './playwright/index.js'
...
test.describe('cssCoverageDir option', () => {
	...
	test('writes coverage to the configured directory', async ({ page, cssCoverage }, testInfo) => {
		await page.setContent('<style>body { margin: 0 }</style><body></body>')
...
test.describe('file content', () => {
	...
	test('writes valid JSON', async ({ page, cssCoverage }, testInfo) => {
		await page.setContent('<style>body { margin: 0 }</style><body></body>')
		filename = expected_filename(testInfo.titlePath)
	})

	test('content is an array of coverage entries', async ({ page, cssCoverage }, testInfo) => {
